...
namespace Dafda.Consuming
{
    internal class LocalMessageDispatcher
    {
        ...
        private MessageRegistration GetMessageRegistrationFor(MessageResult messageResult) =>
            _messageHandlerRegistry.GetRegistrationFor(messageResult.Topic, messageResult.Message.Metadata.Type)
        ...
        public async Task Dispatch(MessageResult messageResult)
        {
            ...
            var messageInstance = messageResult.Message.ReadDataAs(registration.MessageInstanceType);
            var context = new MessageHandlerContext(messageResult.Message.Metadata);
